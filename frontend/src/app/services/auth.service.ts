...
export class AuthService {
  ...
  login(data: LoginRequest): Observable<LoginResponse> {
    return this.http
      .post<LoginResponse>(this.apiUrl, data)
      .pipe(
        tap((response) => {
      ...
      .pipe(
        // Если сервер недоступен — вернуть мок-ответ
        // Вместо catchError можно использовать fallback внутри pipe
        // или подключить `catchError` напрямую
        catchError(() => {
          const mockUser: User = {
            email: 'mock@user.com',
            name: 'Mock Engineer',
